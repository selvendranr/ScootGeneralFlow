...
public class AcceptDateRangeOneway extends TestBaseSetup {
...
String requestInfo="a[title='Request info']";
	...
	String fare="//td[text()='Fare expires']/following-sibling::td[2]";
	String payment="//td[text()='Payment']/following-sibling::td[2]";
	String guest="//td[text()='Guest names']/following-sibling::td[2]";
	...
	String fareV,paymentV,guestV;
	int paymentWithPercent,paymentWithOutPercent,paymentSpilit;
 
	private Login login;
	private WebElement webEle;
	Select select;
	public AcceptDateRangeOneway(WebDriver driver) {
		this.driver = driver;
	}
	...
	try {
		...
		e1.printStackTrace();
		 ...
		 waitExplicitlyByLocator(driver,loadingImg,"cssselector",50);
 		  waitExplicitlyNotByLocator(driver,loadingImg,"cssselector",220);
 	 	}
	 	catch(TimeoutException e){
	 		System.out.println("After Logout Processing is not displayed");
	 	}
	waitExplicitlyByLocator(driver,requestId,"xpath",120);	
	
	 

	
	fareV=findEle(driver,fare,"xpath").getText();
	paymentV=findEle(driver,payment,"xpath").getText();
	guestV=findEle(driver,guest,"xpath").getText();

	int paymentWithPercent = paymentV.length();
	int paymentWithOutPercent = paymentV.replace("%", "").length();
	paymentSpilit=paymentWithPercent-paymentWithOutPercent;
	
	
	System.out.println("Count of "+paymentSpilit); 
		...
		try{
 			 waitExplicitlyByLocator(driver,suceedMsg,"cssselector",30000);
 		 	}
		 	catch(TimeoutException e){
		 		System.out.println("After Logout Processing is not displayed");
		 	}
